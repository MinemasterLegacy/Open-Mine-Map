...
public class WaypointEntryWidget extends ClickableWidget {
    ...
    protected void renderWidget(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.drawTexture(RenderLayer::getGuiTextured, waypoint.identifier, getX() + 3, getY() + 3, 0, 0, 14, 14, 14, 14);

        int xMod = 0;
        setTooltip(null);
        selection = Selection.NONE;
        for (Identifier i : new Identifier[]{editId, visibleWaypoint ? viewOnId : viewOffId, pinnedWaypoint ? pinOnId : pinOffId}) {
            context.drawTexture(RenderLayer::getGuiTextured, i, getX() + width - 17 - (xMod * 16), getY() + 3, 0, 0, 14, 14, 14, 14);
