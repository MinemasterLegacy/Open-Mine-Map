...
public class HudMap {
    ...
    private static BufferedPlayer drawDirectionIndicatorsToMap(DrawContext context, PlayerEntity player, boolean indicatorsOnly) {
        ...
        if (OverlayVisibility.checkPermissionFor(TileManager.showDirectionIndicators, OverlayVisibility.LOCAL) && !Double.isNaN(d))
            DirectionIndicator.draw(RenderPipelines.GUI_TEXTURED, context, d,hudMapX + (hudMapWidth / 2) - 12 + mapXOffset, hudMapY + (hudMapHeight / 2) - 12 + mapYOffset, true, indicatorsOnly);
    ...
    private static void drawBufferedPlayersToMap(DrawContext context, ArrayList<BufferedPlayer> players) {
        for (BufferedPlayer player : players) {
            ...
            context.drawTexture(RenderPipelines.GUI_TEXTURED, player.texture, x, y, 8 + player.leftCrop,8 + player.upCrop, width, height,  width, height, 64, 64);
            context.drawTexture(RenderPipelines.GUI_TEXTURED, player.texture, x, y, 40 + player.leftCrop,8 + player.upCrop, width, height, width, height, 64, 64);
    ...
    public static void render(DrawContext context, RenderTickCounter renderTickCounter) {
        ...
        if (directionIndicator.loadSuccess && OverlayVisibility.checkPermissionFor(TileManager.showDirectionIndicators, OverlayVisibility.SELF))
            DirectionIndicator.draw(RenderPipelines.GUI_TEXTURED, context, PlayerAttributes.geoYaw,hudMapX + (hudMapWidth / 2) - 12, hudMapY + (hudMapHeight / 2) - 12, true, indicatorOnly);
        ...
        if (!Double.isNaN(direction)) { //skip drawing the compass if direction is NaN (it can be separate of long-lat due to the two-point sampling system)
            ...
            //draw the compass
            context.drawTexture(RenderPipelines.GUI_TEXTURED, compassIdentifier, hudCompassX, hudCompassY, (float) (PlayerAttributes.yaw - direction - ((double) hudCompassWidth / 2)) , 0, hudCompassWidth, 16,  hudCompassWidth, 16, 360, 16);
            //draw the snap angle indicator
            if (doSnapAngle) context.drawTexture(RenderPipelines.GUI_TEXTURED, snapAngleIdentifier, hudCompassX, hudCompassY, (float) (PlayerAttributes.yaw + snapAngle - ((double) hudCompassWidth / 2)) , 0, hudCompassWidth, 16, hudCompassWidth, 16, 90, 16);
