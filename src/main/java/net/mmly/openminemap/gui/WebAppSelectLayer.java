...
public class WebAppSelectLayer extends ClickableWidget {
    ...
    protected void drawWidget(DrawContext context) {
        ...
        if (selection != -1) context.drawTexture(iconSelections.get(WebIcon.getUsingId(selection)), getX() + 2 - 1, getY() + 2 + (selection * 16) - 1, 0, 0, 12, 16, 12, 16);

        for (int i = 0; i < webIcons.size(); i++) {
            context.drawTexture(webIcons.get(WebIcon.getUsingId(i)), getX() + 2, getY() + 2 + (i * 16), 0, 0, 10, 14, 10, 14);
        }
    }
    ...
    public void onClick(double mouseX, double mouseY) {
        ...
        int zoom = FullscreenMapScreen.map.getZoom();
        switch (selection) {
            case 0: {
        ...
        try (Writer writer = new BufferedWriter(new OutputStreamWriter(
                ...
                writer.write(
                        "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                                ...
                                "\t\t\t<tilt>0</tilt>\n" +
                                "\t\t\t<range>"+zoomToMetersAbove(FullscreenMapScreen.map.getZoom())+"</range>\n" +
