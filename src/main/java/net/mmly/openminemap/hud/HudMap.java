...
public class HudMap {
    ...
    private static void drawPlayerToMap(DrawContext context, PlayerEntity player) {
        ...
        context.drawTexture(RenderPipelines.GUI_TEXTURED, pTexture, hudMapX + (hudMapWidth / 2) - 4 + mapXOffset + leftCrop, hudMapY + (hudMapHeight / 2) - 4 + mapYOffset + upCrop, 8 + leftCrop, 8 + upCrop, rightCrop - leftCrop, downCrop - upCrop, rightCrop - leftCrop, downCrop - upCrop, 64, 64);
        context.drawTexture(RenderPipelines.GUI_TEXTURED, pTexture, hudMapX + (hudMapWidth / 2) - 4 + mapXOffset + leftCrop, hudMapY + (hudMapHeight / 2) - 4 + mapYOffset + upCrop, 40 + leftCrop, 8 + upCrop, rightCrop - leftCrop, downCrop - upCrop, rightCrop - leftCrop, downCrop - upCrop, 64, 64);

        double d = player.getYaw() - Direction.calcDymaxionAngleDifference();
        if (OverlayVisibility.checkPermissionFor(TileManager.showDirectionIndicators, OverlayVisibility.LOCAL) && !Double.isNaN(d)) DirectionIndicator.draw(RenderPipelines.GUI_TEXTURED, context, d,hudMapX + (hudMapWidth / 2) - 12 + mapXOffset, hudMapY + (hudMapHeight / 2) - 12 + mapYOffset, true);
    }
    ...
    public static void render(DrawContext context, RenderTickCounter renderTickCounter) {
        ...
        for (int i = 0; i < tileIdentifiers.length; i++) {
            for (int j = 0; j < tileIdentifiers[i].length; j++) {
                ...
                context.drawTexture(RenderPipelines.GUI_TEXTURED, tileIdentifiers[i][j], tileX + leftCrop, tileY + topCrop, leftCrop, topCrop, trueHW - rightCrop - leftCrop, trueHW - bottomCrop - topCrop, trueHW, trueHW);
            }
        }
        ...
        if (directionIndicator.loadSuccess && OverlayVisibility.checkPermissionFor(TileManager.showDirectionIndicators, OverlayVisibility.SELF)) DirectionIndicator.draw(RenderPipelines.GUI_TEXTURED, context, PlayerAttributes.geoYaw,hudMapX + (hudMapWidth / 2) - 12, hudMapY + (hudMapHeight / 2) - 12, true);
